...
function Projects() {
  ...
  useEffect(() => {
    axios.get('php/getData.php').then((response) => {
      setListOfProjects(response.data.project)
      setProjectTech(response.data.projectSkill)
    })
  }, [])
      ...
      <div className="scroll_wrap">
        <div className="projects_list">
          {listOfProjects.map((project) => {
            const { id, project_name, description, image_name, url, git } = project
            return (
              <div className="one_project" key={id}>
                <img src={`images/${image_name}`} alt={project_name} />
                <h3>{project_name}</h3>
                <p>{description}</p>
                <div className="one_project_tech">
                  {projectTech
                    .filter((tech) => {
                      return tech.project_id === id
                    })
                    .map((oneTech) => {
                      const { skill_id, description, image_name } = oneTech
                      return <img key={`${id}.${skill_id}`} src={`images/${image_name}`} alt={description} />
                    })}
                </div>
                <div className="one_project_links">
                  <button>
                    <a href={url} target="_blank" rel="noreferrer">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currenctColor" strokeWidth="0.00024000000000000003" xmlns="http://www.w3.org/2000/svg">
                        <g strokeWidth="0" />
                  ...
                  <button>
                    <a href={git} target="blank">
                      <svg fill="currentColor" viewBox="-2.5 0 19 19" stroke="currentColor" xmlns="http://www.w3.org/2000/svg">
                        <g strokeWidth="0" />
